            ...
            <div className="flex-1 min-w-0 h-full">
              {openChatWindow ? (
                ...
                <ChatsSection
                  chatSubTab={chatSubTab}
                  onChatClick={(chat) => {
                    // Extract the actual ID from the conversation ID string
                    let actualId;
                    if (chat.type === 'private') {
                      // For private chats, conversationId is like "private_1", extract the participant ID
                      actualId = parseInt(chat.conversationId.replace('private_', ''));
                    } else {
                      // For group chats, conversationId is like "group_1", extract the group ID
                      actualId = parseInt(chat.conversationId.replace('group_', ''));
                    }
                    ...
                    setOpenChatWindow({
                      ...
                      type: chat.type,
                      name: chat.name,
                      participantId: chat.type === 'private' ? actualId : undefined
                    });
                  }}
