...
export default function GroupFeed({ groupId }: Props) {
  ...
  useEffect(() => {
    ...
    const handleGroupPostUpdate = (event: MessageEvent) => {
      try {
        const message = JSON.parse(event.data)
        if (message.type === 'group_post_update' && message.group_id === parseInt(groupId)) {
        ...
        console.error('Error handling group post update:', error)
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {posts.map(post => {
        ...
        const isAuthorOnline = onlineUsers.some(u => u.user_id === post.user.id && u.is_online)
