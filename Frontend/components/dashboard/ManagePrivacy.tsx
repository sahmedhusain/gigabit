...
import { X, Lock, Globe, Users, User, Check } from 'lucide-react'
...
export default function ManagePrivacy({
  ...
  React.useEffect(() => {
    setPostPrivacy(currentPrivacy)
    setSelectedUsers(currentSelectedUsers)
  }, [currentPrivacy, currentSelectedUsers])
  ...
  const { isLoading, performUpdate } = useOptimisticUpdate(null, {
    ...
    onError: (err: unknown) => {
      ...
      toastError(`Failed to update privacy: ${message}`)
    }
  })

  const handleUpdatePrivacy = async () => {
    if (!isConnected) {
      toastError('Cannot update privacy while offline')
      return
    }
    ...
    performUpdate(
      () => null,
      async () => {
        await onUpdatePrivacy(postPrivacy, selectedUsers)
        return {}
      }
    )
  }
              ...
              <div className="space-y-6">
                ...
                {!isConnected && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                  >
                    <p className="text-red-300 text-sm font-medium">You are currently offline. Privacy settings will be updated when connection is restored.</p>
                  </motion.div>
                )}
              </div>
            </motion.div>
            ...
            >
              <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4">
                ...
                <motion.button
                  ...
                  whileHover={{ scale: (isLoading || !isConnected) ? 1 : 1.05 }}
                  whileTap={{ scale: (isLoading || !isConnected) ? 1 : 0.95 }}
                  transition={{ type: 'spring', stiffness: 400, damping: 17 }}
                >
                  {isLoading ? (
